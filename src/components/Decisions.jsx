import { useRef } from 'react';
import { motion, useInView } from 'framer-motion';
import { decisions } from '../data/decisions';
import SectionHeading from './ui/SectionHeading';
import useReducedMotion from '../hooks/useReducedMotion';

function DecisionCard({ decision, cardVariants }) {
  return (
    <motion.div
      variants={cardVariants}
      className="border border-white/[0.08] hover:border-white/[0.16] transition-colors duration-300 bg-white/[0.05] backdrop-blur-sm rounded-xl2 p-6"
    >
      <h3 className="text-lg font-bold text-text-primary mb-4">{decision.title}</h3>

      <p className="font-mono text-xs uppercase tracking-[0.2em] text-text-muted mb-1">
        The tradeoff
      </p>
      <p className="text-text-muted text-sm mb-4">{decision.tradeoff}</p>

      <p className="font-mono text-xs uppercase tracking-[0.2em] text-text-muted mb-1">
        The call
      </p>
      <p className="text-text-muted text-sm mb-5">{decision.call}</p>

      <span className="inline-flex items-center gap-1.5 bg-emerald-bg text-emerald text-xs px-2.5 py-0.5 rounded-full font-medium">
        {decision.outcome}
      </span>
    </motion.div>
  );
}

export default function Decisions() {
  const ref = useRef(null);
  const isInView = useInView(ref, { once: true, margin: '-80px' });
  const prefersReducedMotion = useReducedMotion();

  const containerVariants = {
    hidden: {},
    visible: {
      transition: {
        staggerChildren: prefersReducedMotion ? 0 : 0.18,
      },
    },
  };

  const cardVariants = {
    hidden: prefersReducedMotion ? { opacity: 0 } : { opacity: 0, y: 40 },
    visible: {
      opacity: 1,
      y: 0,
      transition: { duration: prefersReducedMotion ? 0 : 0.65, ease: 'easeOut' },
    },
  };

  return (
    <section id="decisions" className="relative py-20 px-6 bg-surface">
      <div className="max-w-6xl mx-auto">
        <SectionHeading title="High-Stakes Decisions" number="04" />

        <motion.div
          ref={ref}
          variants={containerVariants}
          initial="hidden"
          animate={isInView ? 'visible' : 'hidden'}
          className="grid grid-cols-1 md:grid-cols-3 gap-6"
        >
          {decisions.map((decision) => (
            <DecisionCard key={decision.title} decision={decision} cardVariants={cardVariants} />
          ))}
        </motion.div>
      </div>
    </section>
  );
}
