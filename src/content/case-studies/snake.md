...
- **Chose pure-function game logic split from rendering** because rules become a headless state machine (`init → update → isGameOver`) that 25 Vitest specs cover in CI. That's the difference between a demo and a product.
...
- **Shipped:** 2026-07-02, the same day the project was green-lit (plan → 4-agent build → tests → deploy)
...
There's a meta-lesson too. After 25 tests were green and a scripted browser bot passed, it took a human play-test to catch this, because the bot clicked buttons instead of trusting the key labels. Each verification layer catches something specific, and each has things it structurally cannot see. All tests green ≠ works.
